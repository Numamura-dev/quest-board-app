...
describe("prisma seedData", () => {
	...
	it("管理画面で確認する主要ステータスのクエストが揃っている", () => {
		...
		for (const status of seedExpectations.adminDashboardStatuses) {
	...
	it("一般ユーザーのマイページ表示に必要な状態が揃っている", () => {
		const participantQuestKeys = seedQuestDefinitions
			.filter((quest) =>
				quest.participants.some(
					(participant) => participant.userKey === "general",
				),
			)
			.map((quest) => quest.key);
		const appliedQuestKeys = seedEntryDefinitions
			.filter(
				(entry) => entry.userKey === "general" && entry.status === "pending",
			)
		...
		expect(appliedQuestKeys).toEqual(
			expect.arrayContaining(
				seedExpectations.generalUserMypage.appliedQuestKeys,
			),
		);
	...
	it("レビューは完了済みクエストに対してのみ紐づいている", () => {
		...
		for (const review of seedReviewDefinitions) {
