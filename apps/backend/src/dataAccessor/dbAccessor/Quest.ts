import type { Quest, QuestParticipant, Reward, User } from "@prisma/client";
...
export class QuestDataAccessor {
	...
	async findById(id: number): Promise<QuestWithRelations | null> {
		const quest = await prisma.quest.findFirst({
			where: {
				id,
				deleted_at: null, // 論理削除されていないクエストのみ取得
				// biome-ignore lint/suspicious/noExplicitAny: Prisma型互換のため
			} as any,
			include: {
				rewards: true, // 報酬情報を含める
				quest_participants: {
	 ...
	 */
	// biome-ignore lint/suspicious/noExplicitAny: Prisma更新データの柔軟な型指定のため
